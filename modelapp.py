...
with col1:
    if toll_plaza_type == "Open Toll System":
        ...
        spots = {1: "Gombak Toll Plaza"}  # Only one spot for Open Toll System (Gombak)
    else:
        st.header("Closed Toll System and Detected Vehicle")
        spots = {1: "Kuala Lumpur", 2: "Penang", 3: "Ipoh"}  # Three spots for Closed Toll System

    results_data = []

    for spot_num, spot_name in spots.items():
        st.subheader(f"Toll Plaza: {spot_name}")
        ...
        if spots[spot_num]:
            with st.spinner(f"Processing {spot_name}..."):
